...
void loop() {
  ...
  {    
    ...
    if (ukDer > 255) ukDer = 255; 
    else if (ukDer < -255) ukDer = -255; 
    ...
    //modulo de saturación
    if (ukIzq > 255) ukIzq = 255; 
    else if (ukIzq < -255) ukIzq = -255; 
  ...
  {  
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, LOW);
    analogWrite(ENA, ukDer);
  }//no es seguro que ENA es el derecho
  else if (ukDer < 0) {
  ...
  if (ukIzq > 0) avanza(ukIzq);
  else if (ukIzq < 0) retrocede(-ukIzq); 
  else para(); 
